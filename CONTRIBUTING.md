...
```bash
git clone https://github.com/nestwork-xyz/web.git nestwork-web
cd nestwork-web
```
...
Nestwork uses a standardized set of GitHub labels for issue triage, project management, pull requests, and release planning. The label definitions are maintained in [.github/scripts/create-gh-labels.sh](./.github/scripts/create-gh-labels.sh)

To apply the standard Nestwork labels to a repository, ensure GitHub CLI is installed and authenticated:
...
✓ Label "bug" deleted from nestwork-xyz/web
✓ Label "documentation" deleted from nestwork-xyz/web
✓ Label "duplicate" deleted from nestwork-xyz/web
✓ Label "good first issue" deleted from nestwork-xyz/web
...
```

This wil create or update the labels used by the Nestwork project, including issue type, priority, status, area, etc.

Projects derived from or inspired by Nestwork may use this script to establish the same labeling conventions. Make sure to update the `REPO="nestwork-xyz/web"` line at the top of the shellscript file to point to your repository.
